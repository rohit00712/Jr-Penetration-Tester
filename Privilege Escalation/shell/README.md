...
1. `Netcat`
2. `Socat`
3. `Metasploit - multi/handle`
4. `Msfvenom` - use for generating payloads.
...
```
2.  PentestMonkey : Reverse Shell Cheatsheet
```http
...
4. Kali Linux also comes pre-installed with a variety of webshells located at `/usr/share/webshells`.
...
listening port less then 1000 we use sudo 
...
Metasploit is listening on a port under 1024. To do this, Metasploit must be run with sudo permissions.
